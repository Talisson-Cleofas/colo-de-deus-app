...
import type { AdminDashboardChange, AdminDashboardData, AdminDashboardLog } from './admin-dashboard.types';
...
export class AdminDashboardService {
  ...
  private demo(): AdminDashboardData {
    ...
    return {
      generatedAt: now,
      metrics: { members: 128, leaders: 21, cells: 18, ministries: 8, events: 14, cenacles: 7, onlineUsers: 5 },
      recentLogs: [
        { id: 'a1', action: 'LOGIN', module: 'AUTH', description: 'Acesso realizado', userName: 'Administrador', userEmail: 'admin@demo.local', createdAt: now },
        { id: 'a2', action: 'CREATE', module: 'MEMBERS', description: 'Novo membro cadastrado', userName: 'Administrador', userEmail: 'admin@demo.local', createdAt: now },
      ],
      latestChanges: [
        { id: 'a2', action: 'CREATE', module: 'MEMBERS', entity: 'Membro', recordId: 'demo-1', description: 'Novo membro cadastrado', userName: 'Administrador', userEmail: 'admin@demo.local', createdAt: now },
      ],
    };
  }
  ...
  private toLog(row: Record<string, string>): AdminDashboardLog {
    return {
      id: row.id || '', action: row.acao || 'CHANGE', module: row.modulo || 'SISTEMA',
      description: row.descricao || `${row.acao || 'Alteração'} em ${row.modulo || 'sistema'}`,
      userName: row.usuario_nome || 'Sistema', userEmail: row.usuario_email || '', createdAt: row.criado_em || '',
    };
  }
  ...
  private onlineUsers(rows: Array<Record<string, string>>): number {
    ...
    return [...state.values()].filter((entry) => entry.action === 'LOGIN' && entry.at >= threshold).length;
  }
  ...
  async getDashboard(): Promise<AdminDashboardData> {
    ...
    const orderedAudit = [...audit].sort((a, b) => (b.criado_em || '').localeCompare(a.criado_em || ''));
    ...
    const latestChanges: AdminDashboardChange[] = orderedAudit
      ...
      .slice(0, 10)
      .map((row) => ({ ...this.toLog(row), entity: row.entidade || '', recordId: row.registro_id || '' }));

    return {
      generatedAt: new Date().toISOString(),
      metrics: {
        members: members.filter((member) => member.active).length,
        leaders: members.filter((member) => member.active && ['DEVELOPER', 'MISSION_LEADER', 'ADMIN', 'MINISTRY_LEADER', 'CELL_LEADER'].includes(member.profile)).length,
