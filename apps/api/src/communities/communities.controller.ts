...
export class CommunitiesController {
 ...
 @Post() @Roles('ADMIN','MINISTRY_LEADER') create(@Body() dto:CreateCommunityDto,@CurrentUser() user:AuthenticatedUser){return this.service.create(dto,user);}
 @Get(':id') detail(@Param('id') id:string,@CurrentUser() user:AuthenticatedUser){return this.service.detail(id,user);}
 @Delete(':id') removeCommunity(@Param('id') id:string,@CurrentUser() user:AuthenticatedUser){return this.service.remove(id,user);}
 @Post(':id/restore') restore(@Param('id') id:string,@CurrentUser() user:AuthenticatedUser){return this.service.restore(id,user);}
 @Patch(':id') update(@Param('id') id:string,@Body() dto:UpdateCommunityDto,@CurrentUser() user:AuthenticatedUser){return this.service.update(id,dto,user);}
 ...
 @Post(':id/attendance') attendance(@Param('id') id:string,@Body() body:{date:string;records:{participantId:string;participantName:string;present:boolean;notes:string}[]},@CurrentUser() user:AuthenticatedUser){return this.service.attendance(id,body.date,body.records,user);}
}
