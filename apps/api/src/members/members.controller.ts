...
export class MembersController {
  constructor(
    ...
    private readonly mapsSync: MapsSyncService,
    private readonly accessProfiles: ProfilesService,
  ) {}
  private visibleTo(user: AuthenticatedUser, member: MemberRow): boolean {
    ...
    if (['MINISTRY_LEADER', 'CELL_LEADER', 'MEMBER'].includes(user.profile)) return true;
    return member.id === user.memberId || member.id === user.id;
  }
  private async scopedMembers(
    user: AuthenticatedUser,
    includeInactive = false,
  ): Promise<MemberRow[]> {
    const members = await this.sheets.listMembers(includeInactive);
    return members.filter((member) => this.visibleTo(user, member));
  }
  @Post()
  ...
  async update(
    ...
    const addressChanged = ['address', 'neighborhood', 'city', 'state', 'zipCode'].some(
      (key) => key in safeDto,
    );
  ...
  async status(@Param('id') id: string, @Body('active') active: boolean) {
    ...
    return {
      member,
      message: active ? 'Membro reativado com sucesso.' : 'Membro desativado com sucesso.',
    };
  }
  @Get('map')
  async map() {
  ...
  async myProfile(@CurrentUser() user: AuthenticatedUser) {
    ...
    const member = members.find(
      (item) => item.id === memberId || item.email === user.email.trim().toLowerCase(),
    );
    ...
    const activeLinks = participants.filter(
      (row) => row.membro_id === member.id && active(row.ativo || ''),
    );
    const ministryMap = new Map<
      string,
      { id: string; name: string; role: string; leader: string }
    >();
    for (const row of ministries.filter((item) => active(item.ativo || ''))) {
      const participant = activeLinks.find(
        (link) => link.tipo === 'MINISTERIO' && link.referencia_id === row.id,
      );
      ...
      if (
        participant ||
        row.lider_id === member.id ||
        row.vice_lider_id === member.id ||
        row.nome === member.ministry
      ) {
        ministryMap.set(row.id, {
          id: row.id,
          name: row.nome,
          role: role || member.role || 'MEMBRO',
          leader: memberById.get(row.lider_id)?.name || '',
        });
      }
    }
    const cellMap = new Map<
      string,
      { id: string; name: string; day: string; time: string; role: string; leader: string }
    >();
    for (const row of cells.filter((item) => active(item.ativo || ''))) {
      const participant = activeLinks.find(
        (link) => link.tipo === 'CELULA' && link.referencia_id === row.id,
      );
      let role = participant?.funcao || '';
      if (row.lider_id === member.id) role = 'LIDER';
      else if (row.vice_lider_id === member.id) role = 'VICE_LIDER';
    ...
    const cenacleMap = new Map<
      string,
      { id: string; name: string; recurrence: string; role: string; leader: string }
    >();
    for (const row of cenacles.filter((item) => active(item.ativo || ''))) {
      const participant = activeLinks.find(
        (link) => link.tipo === 'CENACULO' && link.referencia_id === row.id,
      );
      let role = participant?.funcao || '';
      ...
      if (
        participant ||
        row.responsavel_id === member.id ||
        row.vice_responsavel_id === member.id
      ) {
        cenacleMap.set(row.id, {
          id: row.id,
          name: row.nome,
          recurrence: row.recorrente || '',
          role,
          leader: memberById.get(row.responsavel_id)?.name || '',
        });
      }
    }
    ...
    const leaders = profileMinistries
      ...
      .filter(
        (item, index, items) =>
          items.findIndex(
            (other) => other.name === item.name && other.structureId === item.structureId,
          ) === index,
      );
    ...
    return {
      ...
      links: {
        ...
        cells: profileCells,
        cenacles: profileCenacles,
        leader: primaryLeader,
        leaders,
        formator: member.formator,
      },
    };
  }
  @Patch('me/profile')
  async updateMyProfile(@Body() dto: UpdateMemberDto, @CurrentUser() user: AuthenticatedUser) {
    ...
    const target = members.find(
      (member) =>
        ...
        member.id === user.id ||
        member.email === user.email.trim().toLowerCase(),
    );
  ...
  async list(
    ...
    const members = (await this.scopedMembers(user)).filter((m) => {
      const haystack = normalize(
        [m.name, m.email, m.ministry, m.cell, m.role, m.city, m.formator].join(' '),
      );
      const statusMatch =
        status === 'all' ||
  ...
  async trash(@CurrentUser() user: AuthenticatedUser) {
    ...
    const rows = (await this.sheets.read('Membros')).filter((r) => Boolean(r.deleted_at));
    return {
      members: rows.map((r) => ({
        id: r.id,
        name: r.nome,
  ...
  async remove(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser) {
    ...
    await this.sheets.softDeleteRecord('Membros', id, user.memberId || user.id);
    return { success: true, message: 'Membro movido para a lixeira.' };
  }
  @Post(':id/restore')
  async restore(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser) {
    if (!['DEVELOPER', 'MISSION_LEADER', 'ADMIN'].includes(user.profile))
      throw new NotFoundException('Membro não encontrado.');
    await this.sheets.restoreRecord('Membros', id, user.memberId || user.id);
    return { success: true, message: 'Membro restaurado com sucesso.' };
  }
  @Get('facets')
  async facets(@CurrentUser() user: AuthenticatedUser) {
    ...
    return {
      ministries: unique(members.map((m) => m.ministry)),
      cells: unique(members.map((m) => m.cell)),
      roles: unique(members.map((m) => m.role)),
    };
  }
  @Get('ministries')
  async ministries(@CurrentUser() user: AuthenticatedUser) {
    ...
    for (const member of members) {
      const key = member.ministry || 'Sem ministério';
    ...
    return [...map.entries()]
      .map(([name, people]) => ({
        name,
        count: people.length,
        leaders: people.filter((p) => /coordenador|líder|lider/i.test(p.role)),
        members: people,
      }))
      .sort((a, b) => b.count - a.count);
  }
  @Get(':id/profile-complete')
  async completeProfile(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser) {
    return this.profiles.complete(id, user);
  }
  @Get(':id/public-profile')
  async publicProfile(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser) {
    return this.profiles.publicProfile(id, user);
  }
  @Patch(':id/admin')
  @Roles('ADMIN', 'MISSION_LEADER', 'DEVELOPER')
  async adminUpdate(
