...
} from '../persistence/interfaces/ministry-repository.interface';
import { StructureSyncService } from '../google/structure-sync.service';
import {
  AddMinistryMemberDto,
  CreateMinistryDto,
  RegisterMinistryAttendanceDto,
  UpdateMinistryDto,
} from './ministry.dto';
import type {
  MinistryAttendanceRecord,
  MinistryMemberRecord,
  MinistryRecord,
...
export class MinistriesService {
  ...
  constructor(
    @Inject(MINISTRY_REPOSITORY) private readonly sheets: IMinistryRepository,
  ...
  private parse(
    ...
    byId: Map<string, { id: string; email: string; name: string }>,
    membersCount = 0,
  ): MinistryRecord {
    ...
    return {
      ...
      code: row.codigo || this.normalizeCode(row.tipo || row.nome || ''),
      name: row.nome || '',
      description: row.descricao || '',
      leaderId: leader?.id || row.lider_id || '',
      leaderEmail: leader?.email || '',
      leaderName: leader?.name || '',
      viceLeaderId: vice?.id || row.vice_lider_id || '',
      viceLeaderEmail: vice?.email || '',
      viceLeaderName: vice?.name || '',
      color: row.cor || '',
      icon: row.icone || '',
      type: row.tipo || 'OUTRO',
  ...
  private canManage(user: AuthenticatedUser, ministry: MinistryRecord): boolean {
    return (
      ...
      (user.profile === 'MINISTRY_LEADER' && ministry.leaderId === user.memberId)
    );
  }
  private assertCanManage(user: AuthenticatedUser, ministry: MinistryRecord) {
    if (!this.canManage(user, ministry))
      throw new ForbiddenException('Você só pode gerenciar o ministério no qual é líder.');
  }
  async list(user: AuthenticatedUser, includeInactive = false): Promise<MinistryRecord[]> {
    ...
    participants
      .filter((row) => row.tipo === 'MINISTERIO' && this.sheets.parseActive(row.ativo || ''))
      .forEach((row) => counts.set(row.referencia_id, (counts.get(row.referencia_id) || 0) + 1));
    let items = (await this.sheets.read(this.tab))
      .map((row) => this.parse(row, byId, counts.get(row.id) || 0))
      .filter((item) => item.id && item.name);
    if (user.profile === 'MINISTRY_LEADER')
      items = items.filter(
        (item) => item.leaderId === user.memberId || item.viceLeaderId === user.memberId,
      );
    return items
  ...
  private async getById(id: string): Promise<MinistryRecord> {
    ...
    const count = participants.filter(
      (item) =>
        item.tipo === 'MINISTERIO' &&
        item.referencia_id === id &&
        this.sheets.parseActive(item.ativo || ''),
    ).length;
  ...
  async detail(id: string, user: AuthenticatedUser) {
    ...
    const [members, attendances] = await Promise.all([
      this.listMembers(id, user),
      this.listAttendance(id, user),
    ]);
  ...
  async create(dto: CreateMinistryDto): Promise<MinistryRecord> {
    ...
    if (
      allRows.some(
        (item) =>
          (item.nome || '').trim().toLocaleLowerCase('pt-BR') === name.toLocaleLowerCase('pt-BR'),
      )
    )
    ...
    const vice = dto.viceLeaderEmail
      ? byEmail.get(dto.viceLeaderEmail.trim().toLowerCase())
      : undefined;
    this.assertVocationalEligibility(leader, true);
    this.assertVocationalEligibility(vice, true);
    const now = new Date().toISOString();
    const id = randomUUID();
    await this.sheets.appendRecord(this.tab, {
      id,
      missao_id: dto.missionId || 'missao-brasilia',
      codigo: this.normalizeCode(dto.code || dto.type || name),
      nome: name,
      descricao: dto.description?.trim() || '',
      lider_id: leader?.id || '',
      vice_lider_id: vice?.id || '',
      cor: dto.color?.trim() || '#9e6939',
      icone: dto.icon?.trim() || '',
      tipo: dto.type?.trim() || 'OUTRO',
      ordem: 0,
      ativo: 'TRUE',
    ...
    if (leader) await this.upsertParticipant(id, leader.id, 'LIDER', 'ADMINISTRACAO');
    if (vice) await this.upsertParticipant(id, vice.id, 'VICE_LIDER', 'ADMINISTRACAO');
  ...
  async update(
    ...
    if (dto.name) {
      ...
      if (
        allRows.some(
          (item) =>
            item.id !== id && (item.nome || '').trim().toLocaleLowerCase('pt-BR') === normalized,
        )
      )
        throw new ConflictException('Já existe um ministério com este nome.');
    }
    ...
    const { byEmail } = await this.context();
    const leaderEmail =
      dto.leaderEmail !== undefined ? dto.leaderEmail.trim().toLowerCase() : current.leaderEmail;
    const viceLeaderEmail =
      dto.viceLeaderEmail !== undefined
        ? dto.viceLeaderEmail.trim().toLowerCase()
        : current.viceLeaderEmail;
    ...
    await this.sheets.updateRecord(this.tab, 'id', id, {
      ...
      codigo: this.normalizeCode(
        dto.code ?? current.code ?? dto.type ?? current.type ?? current.name,
      ),
    ...
    if (isAdmin && leader)
      await this.upsertParticipant(id, leader.id, 'LIDER', user.memberId || user.id);
    if (isAdmin && vice)
      await this.upsertParticipant(id, vice.id, 'VICE_LIDER', user.memberId || user.id);
    if (isAdmin) await this.sync.reconcileStructure('MINISTERIO', id);
    return this.getById(id);
  }
  async deactivate(id: string, user: AuthenticatedUser): Promise<void> {
    await this.update(id, { active: false }, user);
  }
    ...
    return participants
      .filter(
        (row) =>
          ...
          this.sheets.parseActive(row.ativo || ''),
      )
      .map((row) => {
        ...
        return {
          ...
          name: member?.name || 'Membro não encontrado',
          email: member?.email || '',
          photo: member?.photo || '',
          role: member?.role || '',
          profile: member?.profile || 'MEMBER',
          function: row.funcao || 'MEMBRO',
          joinedAt: row.data_entrada || row.criado_em || '',
  ...
  private async upsertParticipant(
    ...
    const member = (await this.sheets.listMembers()).find((item) => item.id === memberId);
    this.assertVocationalEligibility(member, Boolean(authorizedBy));
    const rows = await this.sheets.read('Participantes');
    const existing = rows.find(
      (row) =>
        row.tipo === 'MINISTERIO' && row.referencia_id === ministryId && row.membro_id === memberId,
    );
    const now = new Date().toISOString();
    const authorization = {
      autorizacao_ano_2_por: isVocationalYear(member?.vocationalYear, 'ANO_2')
        ? authorizedBy
        : '',
      autorizacao_ano_2_em: isVocationalYear(member?.vocationalYear, 'ANO_2') ? now : '',
    };
    if (existing)
  ...
  async addMember(id: string, dto: AddMinistryMemberDto, user: AuthenticatedUser) {
    ...
    const member = (await this.sheets.listMembers()).find(
      (item) => item.id === dto.memberId && item.active,
    );
    if (!member) throw new NotFoundException('Membro ativo não encontrado.');
    this.assertVocationalEligibility(member, true);
    await this.upsertParticipant(
      id,
      dto.memberId,
      dto.function || 'MEMBRO',
      isVocationalYear(member.vocationalYear, 'ANO_2') ? user.memberId || user.id : '',
    );
    return this.listMembers(id, user);
  }
  private assertVocationalEligibility(member?: { vocationalYear?: string }, allowYearTwo = false) {
    if (
      member &&
      !canParticipateInMinistries(member.vocationalYear) &&
      !(allowYearTwo && isVocationalYear(member.vocationalYear, 'ANO_2'))
    )
      throw new ForbiddenException(
        'Membros do Ano 1 não podem ser vinculados a ministérios. Ano 2 exige autorização da liderança.',
      );
  }
  async removeMember(id: string, memberId: string, user: AuthenticatedUser) {
    ...
    if ((isLeader || isViceLeader) && !isAdmin) {
      throw new ConflictException(
        'Somente a administração pode remover líder ou vice-líder do ministério.',
      );
    }
    ...
    const linkedRows = rows.filter(
      (item) =>
        item.tipo === 'MINISTERIO' &&
        item.referencia_id === id &&
        item.membro_id === memberId &&
        this.sheets.parseActive(item.ativo || ''),
    );
    if (!linkedRows.length && !isLeader && !isViceLeader)
  ...
  async listAttendance(id: string, user: AuthenticatedUser): Promise<MinistryAttendanceRecord[]> {
    ...
    return attendances
      .filter((row) => row.tipo === 'MINISTERIO' && row.referencia_id === id)
      .map((row) => ({
        id: row.id,
        memberId: row.membro_id,
        memberName: byId.get(row.membro_id)?.name || 'Membro não encontrado',
        date: row.data || '',
        present: this.sheets.parseActive(row.presente || ''),
  ...
  async registerAttendance(
    id: string,
    dto: RegisterMinistryAttendanceDto,
    user: AuthenticatedUser,
  ) {
    const ministry = await this.getById(id);
    this.assertCanManage(user, ministry);
    const participant = (await this.listMembers(id, user)).find(
    ...
    const existing = rows.find(
      (row) =>
        row.tipo === 'MINISTERIO' &&
        row.referencia_id === id &&
        row.membro_id === dto.memberId &&
        row.data === dto.date,
    );
    const now = new Date().toISOString();
    const record = {
      membro_id: dto.memberId,
      tipo: 'MINISTERIO',
      referencia_id: id,
      evento_id: '',
      data: dto.date,
      presente: dto.present ? 'TRUE' : 'FALSE',
      justificativa: dto.justification?.trim() || '',
      registrado_por: user.memberId,
      origem: 'LIDER',
      atualizado_em: now,
    };
    if (existing)
      await this.sheets.updateRecord('Presenças', 'id', existing.id, { ...existing, ...record });
    else
      await this.sheets.appendRecord('Presenças', { id: randomUUID(), ...record, criado_em: now });
