import { BadRequestException, Inject, ForbiddenException, Injectable, NotFoundException, OnModuleDestroy, OnModuleInit } from '@nestjs/common';
...
export class NotificationsService implements OnModuleInit,OnModuleDestroy {
 ...
 private canManageCenacle(user:AuthenticatedUser,cenacle:SheetRecord){const uid=this.uid(user);if(['DEVELOPER','MISSION_LEADER','ADMIN'].includes(user.profile))return true;return [cenacle.responsavel_id,cenacle.vice_responsavel_id].includes(uid);}
 async create(dto:CreateNotificationDto,user:AuthenticatedUser){
  if(!this.canCreate(user))throw new ForbiddenException('Você não possui permissão para enviar notificações.');
  if(dto.audience==='CENACULO'){
   if(!dto.audienceId)throw new BadRequestException('Informe o cenáculo destinatário.');
   const cenacle=(await this.rows('Cenáculos')).find(row=>row.id===dto.audienceId&&(!row.ativo||truthy(row.ativo)));
   if(!cenacle)throw new BadRequestException('Cenáculo destinatário inválido.');
   if(!this.canManageCenacle(user,cenacle))throw new ForbiddenException('Você não pode enviar notificações para este cenáculo.');
  }
  return this.createRecord(dto,{id:this.uid(user),name:user.name});
 }
