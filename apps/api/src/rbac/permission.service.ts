...
import { DEFAULT_PROFILE_PERMISSIONS, PROFILE_PERMISSION_CEILINGS } from './permission.defaults';
...
export class PermissionService {
  constructor(
    ...
    private readonly ministryModules: MinistryModuleService,
  ) {}
  ...
  normalizeProfile(value: string): ProfileCode {
    ...
    if (['MISSION_LEADER', 'LIDER_MISSAO', 'LIDER MISSAO', 'ADMIN'].includes(code))
      return ProfileCode.MISSION_LEADER;
    if (['MINISTRY_LEADER', 'LIDER_MINISTERIO', 'LIDER DE MINISTERIO'].includes(code))
      return ProfileCode.MINISTRY_LEADER;
    if (['CELL_LEADER', 'LIDER', 'LEADER', 'LIDER_CELULA', 'LIDER DE CELULA'].includes(code))
      return ProfileCode.CELL_LEADER;
  ...
  private defaults(profile: ProfileCode): UserPermissions {
    const rows = DEFAULT_PROFILE_PERMISSIONS.filter(
      (row) => row.profileCode === profile && row.allowed,
    );
    return {
      profile,
  ...
  async forProfile(profileInput: string): Promise<UserPermissions> {
    ...
    return this.cache.remember(
      `rbac:profile:${profile}`,
      5 * 60_000,
      async () => {
        const [rows, permissionCatalog] = await Promise.all([
        ...
        const parsed: ProfilePermission[] = rows
          .filter(
            (r) =>
              r.profileCode === profile && r.active !== false && activeCodes.has(r.permissionCode),
          )
          .map((r) => ({
            profileCode: profile,
        ...
        if (!parsed.length) return this.defaults(profile);
        const ceiling = PROFILE_PERMISSION_CEILINGS[profile];
        const allowed = parsed.filter(
          (row) => row.allowed && (!ceiling || ceiling.includes(row.permissionCode)),
        );
        return {
          profile,
          permissions: allowed.map((row) => row.permissionCode),
          scopes: Object.fromEntries(allowed.map((row) => [row.permissionCode, row.scope])),
        };
      },
      ['rbac'],
      30 * 60_000,
  ...
  async forUser(user: AuthenticatedUser): Promise<UserPermissions> {
    const base = await this.forProfile(user.profile);
    return { ...base, ministryModules: await this.ministryModules.modulesForUser(user) };
  }
  ...
  async catalog() {
    ...
    return { profiles, permissions, profilePermissions };
  }
}
