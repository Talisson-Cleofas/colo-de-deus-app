...
export class PermissionsService {
  ...
  async list(seed=true){if(this.sheets.isDemo())return [...this.demoPermissions].sort((a,b)=>a.code.localeCompare(b.code));let rows=await this.sheets.read('Permissoes');if(seed&&!rows.length){await this.seed();rows=await this.sheets.read('Permissoes');}return rows.map(r=>this.map(r)).filter(p=>p.code&&p.resource&&p.action).sort((a,b)=>a.code.localeCompare(b.code));}
