...
export class SomaController {
 ...
 @Patch('settings') @RequirePermissions(Permission.SOMA_WRITE) @RequireMinistryModule('FINANCAS') updateSettings(@Body() body:Record<string,unknown>,@CurrentUser() user:AuthenticatedUser){return this.service.updateSettings(body,user)}
 ...
 @Get('financial/report') @RequirePermissions(Permission.FINANCIAL_REPORT_READ) @RequireMinistryModule('FINANCAS') financialReport(@CurrentUser() user:AuthenticatedUser,@Query('from') from?:string,@Query('to') to?:string,@Query('status') status?:string,@Query('method') method?:string){return this.service.financialReport(user,{from,to,status,method})}
 @Get('financial/export/:format') @RequirePermissions(Permission.FINANCIAL_REPORT_READ) @RequireMinistryModule('FINANCAS') async export(@Param('format') format:'csv'|'xls',@CurrentUser() user:AuthenticatedUser,@Query('from') from:string|undefined,@Query('to') to:string|undefined,@Res() res:Response){const file=await this.service.exportReport(user,format,{from,to});res.setHeader('Content-Type',format==='xls'?'application/vnd.ms-excel':'text/csv; charset=utf-8');res.setHeader('Content-Disposition',`attachment; filename="relatorio-financeiro.${format}"`);res.send(file)}
