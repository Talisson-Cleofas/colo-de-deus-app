...
export function CommunityDetailPage() {
  ...
  const load = async () => {
    try {
      const [cr, mr, mins, cellRes] = await Promise.all([
        api.get<Community>(`/communities/${id}`),
        api.get<{ members: Member[] }>('/members', { params: { status: 'active' } }),
        api.get<Ministry[]>('/ministries'),
        api.get<Community[]>('/communities', { params: { type: 'CELL' } }),
      ]);
      setC(cr.data);
      setMembers(mr.data.members);
      setMinistries(mins.data);
      setCells(cellRes.data.map((x) => ({ id: x.id, name: x.name })));
      const leader = cr.data.leader,
        vice = cr.data.coLeaders[0];
      setForm({
        name: cr.data.name,
        description: cr.data.description,
        leaderId: leader.external ? '' : leader.id,
        leaderName: leader.name === 'Sem líder' ? '' : leader.name,
        leaderContact: leader.phone || '',
        viceLeaderId: vice?.external ? '' : vice?.id || '',
        viceLeaderName: vice?.name || '',
        viceLeaderContact: vice?.phone || '',
        modality: cr.data.modality || 'Presencial',
        ministryId: cr.data.ministryId,
        cellId: cr.data.cellId || '',
        weekday: cr.data.weekday,
        time: cr.data.time,
        startDate: cr.data.startDate || '',
        endDate: cr.data.endDate || cr.data.startDate || '',
        endTime: cr.data.endTime || cr.data.time || '',
        recurrence: 'NAO',
        address: cr.data.address,
        neighborhood: cr.data.neighborhood,
        city: cr.data.city,
        state: cr.data.state,
        latitude: cr.data.latitude,
        longitude: cr.data.longitude,
        type: cr.data.type,
        active: cr.data.active,
      });
    } catch (e) {
  ...
  const singular = isCell ? 'célula' : 'cenáculo';
  const action = async (path: string) => {
    try {
      await api.post(`/communities/${c.id}/${path}`);
      await load();
    } catch (e) {
      setError(apiErrorMessage(e));
    }
  ...
  const duplicate = async () => {
    try {
      const r = await api.post<Community>(`/communities/${c.id}/duplicate`, duplicateForm);
      setDuplicateOpen(false);
      nav(`/comunidades/${r.data.id}`);
    } catch (e) {
      setError(apiErrorMessage(e));
    }
  };
  const save = async () => {
    try {
      await api.patch(`/communities/${c.id}`, form);
      setEditOpen(false);
      await load();
    } catch (e) {
      setError(apiErrorMessage(e));
  ...
  const remove = async (pid: string) => {
    try {
      await api.delete(`/communities/${c.id}/participants/${pid}`);
      await load();
    } catch (e) {
      setError(apiErrorMessage(e));
    }
  ...
  return (
    <Box>
      ...
      <Card sx={{ p: { xs: 2.5, md: 4 }, mt: 2 }}>
        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" gap={3}>
          <Box>
            <Stack direction="row" gap={1}>
              <Chip label={isCell ? 'Célula' : 'Cenáculo'} color="primary" />
              <Chip label={c.ministryName || 'Sem ministério'} variant="outlined" />
              {isCell && c.modality && <Chip label={c.modality} variant="outlined" />}
              {!isCell && c.cellName && <Chip label={`Célula: ${c.cellName}`} variant="outlined" />}
            </Stack>
          ...
          <Stack>
            ...
            {c.canEdit && (
              <Button
                ...
                onClick={() => nav(`/comunidades/${c.id}/presenca`)}
              >
                Registrar presença
              </Button>
            )}
            ...
            {!isCell && c.canEdit && c.status === 'UPCOMING' && (
              <Button
                color="warning"
            ...
            {!isCell && c.canEdit && c.status === 'UPCOMING' && (
              <Button color="error" startIcon={<CancelOutlined />} onClick={() => action('cancel')}>
                Cancelar
              </Button>
            )}
            {!isCell && c.canEdit && c.status === 'FINISHED' && (
              <Button startIcon={<ReplayOutlined />} onClick={() => action('reopen')}>
                Reabrir
              </Button>
            )}
            {isCell && (
              <Button
                ...
                onClick={() => nav(`/mapa-celulas?selected=${c.id}`)}
              >
                Ver no mapa
              </Button>
            )}
        ...
        >
          <Card variant="outlined" sx={{ p: 2 }}>
            <Typography fontWeight={700}>
              {isCell ? 'Encontros' : 'Agenda e recorrência'}
            </Typography>
            <Typography color="text.secondary">
              {isCell
                ? `${c.weekday || 'Dia não definido'}, ${c.time || 'horário não definido'}`
                : `${c.startDate || 'Data não definida'} ${c.time || ''} até ${c.endDate || c.startDate || ''} ${c.endTime || ''}`}
            </Typography>
            {!isCell && (
              <Chip
                size="small"
          ...
          <Card variant="outlined" sx={{ p: 2 }}>
            ...
            <Typography color="text.secondary">
              {[c.address, c.neighborhood, c.city && `${c.city}/${c.state}`]
                .filter(Boolean)
                .join(', ') || 'Não informada'}
            </Typography>
          </Card>
          <Card variant="outlined" sx={{ p: 2 }}>
            ...
            <Typography color="text.secondary">{c.participants.length} pessoas</Typography>
          </Card>
        </Box>
        <Typography variant="h5" mt={4}>
          {isCell ? 'Liderança' : 'Responsáveis'}
        </Typography>
        <Stack direction="row" gap={2} flexWrap="wrap" mt={2}>
          {[c.leader, ...c.coLeaders]
            .filter((p) => p.id)
            .map((p) => (
              <Card key={p.id} variant="outlined" sx={{ p: 2, minWidth: 260 }}>
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center" mt={4}>
          ...
          {c.canManageParticipants && (
            <Button startIcon={<PersonAddAltOutlined />} onClick={() => setAddOpen(true)}>
          ...
          {c.participants.map((p) => (
            <Card key={p.id} variant="outlined" sx={{ p: 1.5 }}>
              <Stack direction="row" gap={1.5} alignItems="center">
                ...
                {c.canManageParticipants &&
                  !isMember &&
                  ![c.leader.id, ...c.coLeaders.map((v) => v.id)].includes(p.id) && (
                    <Button color="error" size="small" onClick={() => remove(p.id)}>
                      <DeleteOutline />
                    </Button>
                  )}
              </Stack>
        ...
        {c.canEdit && (
          <>
            ...
            <Typography color="text.secondary" mb={2}>
              Envie fotos diretamente da galeria, câmera ou armazenamento do dispositivo.
            </Typography>
            <Box maxWidth={460}>
              <FileUploader
                referenceId={c.id}
                category={isCell ? 'CELL_FILE' : 'CENACLE_FILE'}
                label="Selecionar arquivo do dispositivo"
                onUploaded={() => setError('')}
              />
            </Box>
          </>
        ...
        <DialogContent dividers>
          <Stack spacing={2}>
            ...
            {isCell ? (
              <>
                <Alert severity="info">
                  Selecione um membro ou digite o nome da liderança externa.
                </Alert>
                <Box
                  sx={{
                    display: 'grid',
                ...
                >
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField {...params} required label="Líder cadastrado ou externo" />
                    )}
                  />
                  <TextField
      ...
      <Dialog open={duplicateOpen} onClose={() => setDuplicateOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>Duplicar cenáculo</DialogTitle>
        <DialogContent dividers>
          <Alert severity="info" sx={{ mb: 2 }}>
            Responsáveis, participantes, local e vínculos serão copiados. Informe apenas a nova data
            e horário.
          </Alert>
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
            <TextField
              ...
              onChange={(e) =>
                setDuplicateForm({
                  ...
                  endDate: duplicateForm.endDate || e.target.value,
                })
              }
              InputLabelProps={{ shrink: true }}
            />
            <TextField
              ...
              type="time"
              label="Hora inicial"
              value={duplicateForm.time}
              onChange={(e) => setDuplicateForm({ ...duplicateForm, time: e.target.value })}
              InputLabelProps={{ shrink: true }}
            />
            <TextField
              required
              ...
              onChange={(e) => setDuplicateForm({ ...duplicateForm, endDate: e.target.value })}
              InputLabelProps={{ shrink: true }}
            />
            <TextField
              required
