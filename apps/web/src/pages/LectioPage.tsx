...
export function LectioPage() {
  ...
  const isDeveloper = user?.profile === 'DEVELOPER';
  ...
  const loadTechnical=async()=>{if(!isDeveloper)return;try{const [a,b,c]=await Promise.all([api.get<LectioSettings>('/lectio/settings'),api.get<LectioSyncLog[]>('/lectio/sync-logs'),api.get<LectioProviderStatus[]>('/lectio/providers/status')]);setSettings(a.data);setLogs(b.data);setProviders(c.data);}catch(e){setError(apiErrorMessage(e));}};
  useEffect(()=>{void load();void loadTechnical();},[isDeveloper]);
  useEffect(()=>{
  ...
  return <Box>
    ...
    {tab===0&&(selected?<Stack gap={2}>
      <Card><CardContent sx={{p:{xs:3,md:5}}}><Stack direction={{xs:'column',md:'row'}} justifyContent="space-between" gap={2}>
        ...
        <Stack alignItems={{xs:'stretch',md:'flex-end'}} gap={1}>{isDeveloper&&<Chip label={selected.status} color={selected.status==='MANUAL'||selected.status==='REVISADA'?'warning':'default'}/>} {isDeveloper&&selected.protected&&<Chip label="Protegida da limpeza" color="success" variant="outlined"/>}{isAdmin&&<Button startIcon={<EditOutlined/>} onClick={()=>{setEditing(selected);setEditorOpen(true)}}>Editar</Button>}</Stack>
