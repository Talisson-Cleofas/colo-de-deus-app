...
const defaultProfileOptions: AccessProfileOption[] = [
  ...
  {
    code: 'MISSION_LEADER',
    name: 'Líder da Missão',
    description: 'Administração funcional da missão.',
    level: 90,
    active: true,
  },
  {
...
export function MembersPage() {
  ...
  const [status, setStatus] = useState(isAdmin ? 'all' : 'active');
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [m, f] = await Promise.all([
        api.get('/members', { params: { status: 'all' } }),
        api.get('/members/facets'),
      ]);
  ...
  const loadAccessProfiles = useCallback(async () => {
    ...
    try {
      ...
      setAccessProfiles(
        valid.length
          ? valid
          : defaultProfileOptions.filter(
              (item) => item.code !== 'DEVELOPER' || user?.profile === 'DEVELOPER',
            ),
      );
    } catch (e) {
      ...
      setAccessProfiles(
        defaultProfileOptions.filter(
          (item) =>
            ...
            (item.code !== 'DEVELOPER' && (item.level ?? 0) <= actorLevel),
        ),
      );
    } finally {
  ...
  useEffect(() => {
    if (isAdmin) void loadAccessProfiles();
  }, [isAdmin, loadAccessProfiles]);
  useEffect(() => {
    setPage(0);
  }, [query, ministry, cell, profile, status]);
  ...
  const profileLabel = useCallback(
    (code: string) => profileNameMap.get(code) || code || 'Perfil não informado',
    [profileNameMap],
  );
  const filterProfileOptions = useMemo(() => {
    ...
    for (const code of codes) {
      if (!options.some((item) => item.code === code))
        options.push({
          code,
          name: profileLabel(code),
          description: 'Perfil presente em cadastro existente.',
          level: 0,
          active: true,
        });
    }
    return options.sort(
      (a, b) => (b.level ?? 0) - (a.level ?? 0) || a.name.localeCompare(b.name, 'pt-BR'),
    );
  ...
  const canEdit = (_member: Member) => isAdmin;

  const submit = async () => {
    setFormError('');
    setSuccess('');
    if (!form.name.trim() || !form.email.trim()) {
      setFormError('Informe pelo menos o nome e o e-mail.');
      return;
    }
    setSaving(true);
    try {
      ...
      setMembers((current) =>
        editing
          ? current.map((m) => (m.id === data.member.id ? data.member : m))
          : [data.member, ...current],
      );
  ...
  const removeMember = async (member: Member) => {
    ...
    try {
      ...
      setSuccess('Membro movido para a lixeira.');
    } catch (e) {
      setError(apiErrorMessage(e));
    }
  };
  const toggleStatus = async (member: Member) => {
    setError('');
    try {
      const { data } = await api.patch<{ member: Member; message: string }>(
  ...
  return (
    <Box>
      <Stack direction={{ xs: 'column', lg: 'row' }} justifyContent="space-between" gap={2} mb={2}>
        <Box>
          <Typography variant="h4">Área de Membros</Typography>
          <Typography color="text.secondary">
            Cadastros, vínculos, perfis e situação dos membros da missão.
          </Typography>
        </Box>
        {isAdmin && (
      ...
      <Paper sx={{ mb: 3 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} variant="scrollable">
          <Tab icon={<GroupsOutlined />} iconPosition="start" label="Listagem" />
          <Tab icon={<MapOutlined />} iconPosition="start" label="Mapa de membros" />
        </Tabs>
      </Paper>
      ...
      {loading ? (
        ...
        <MembersMapPanel members={members.filter((m) => m.active)} />
      ) : (
        <>
          <Paper sx={{ p: 2, mb: 2 }}>
            <Stack direction={{ xs: 'column', xl: 'row' }} spacing={1.5}>
              <TextField
                ...
                fullWidth
                placeholder="Buscar por nome, e-mail, função, cidade ou formador"
                slotProps={{
                  input: {
                    startAdornment: (
                  ...
                  <TableRow key={member.id} hover>
                    <TableCell>
                      <Stack direction="row" spacing={1.5} alignItems="center">
                        <Avatar src={member.photo}>{member.name.slice(0, 1)}</Avatar>
                        <Box>
                    ...
                    <TableCell>
                      <Chip size="small" label={profileLabel(member.profile)} />
                    </TableCell>
                    <TableCell>{member.ministry || '—'}</TableCell>
                    <TableCell>{member.cell || '—'}</TableCell>
                    <TableCell>{member.formator || '—'}</TableCell>
                    <TableCell>
                      <Chip
                        size="small"
                      ...
                      <Stack direction="row" justifyContent="flex-end">
                        ...
                        {isAdmin && (
                          <Tooltip title="Excluir">
                            <IconButton color="error" onClick={() => void removeMember(member)}>
                              <DeleteOutline />
                            </IconButton>
                          </Tooltip>
                        )}
                        {isAdmin && (
                          <Tooltip title={member.active ? 'Desativar' : 'Reativar'}>
                            <IconButton
                              color={member.active ? 'warning' : 'success'}
                              onClick={() => void toggleStatus(member)}
                            >
                              {member.active ? <ToggleOnOutlined /> : <ToggleOffOutlined />}
                            </IconButton>
                          </Tooltip>
                        )}
                      </Stack>
        ...
        <DialogContent dividers>
          ...
          >
            <TextField
              ...
              disabled={!isAdmin && Boolean(editing)}
            />
            <TextField
              required
              type="email"
            ...
            <TextField
              ...
              label="Ministério"
              value={form.ministry}
              onChange={(e) => setField('ministry', e.target.value)}
              disabled={!isAdmin}
            >
            ...
            <TextField
              label="Formador"
              value={form.formator}
              onChange={(e) => setField('formator', e.target.value)}
              disabled={!isAdmin}
            />
            <TextField
              label="Dons (separados por vírgula)"
              value={form.gifts}
              onChange={(e) => setField('gifts', e.target.value)}
              sx={{ gridColumn: { md: '1 / -1' } }}
            />
            <TextField
              label="Biografia / observações"
              value={form.bio}
