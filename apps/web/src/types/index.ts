export type KnownAccessProfile =
...
export type MinistryAttendance = {
  id: string;
  memberId: string;
  memberName: string;
  date: string;
...
export type LectioSettings = {
  primarySource: 'CNBB' | 'CANCAO_NOVA';
  fallbackSource: 'CNBB' | 'CANCAO_NOVA';
  cnbbEnabled: boolean;
...
export type LectioProviderAttempt = {
  source: 'CNBB' | 'CANCAO_NOVA';
  priority: number;
...
export type LectioSyncResult = {
  ...
  attempts: number;
  providerErrors: Partial<Record<'CNBB' | 'CANCAO_NOVA', string>>;
...
export type EventManagementOptions = {
  ...
  cells: { id: string; name: string; ministryId: string }[];
  cenacles: { id: string; name: string; ministryId: string; cellId: string }[];
};
...
export type MissionaryAgendaType =
  'EVANGELIZACAO' | 'MISSAO' | 'VISITA' | 'FORMACAO' | 'RETIRO' | 'OUTRO';
export type MissionaryAgendaStatus =
  | 'RASCUNHO'
